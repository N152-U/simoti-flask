...
def get_user(id):
    ...
    if has_access:
        try:
            patient = PatientModel.get_patient(id)
            if patient != None:
...
def get_detail_user(id):
    ...
    if has_access:
        try:
            patient = PatientModel.get_detail_patient(id)
            if patient != None:
                return jsonify(patient)
            else:
                return jsonify({}), 404
    ...
    if has_access:
        try:
            ...
            if affected_rows == 1:
                ...
                return jsonify({"message": "No patient deleted"}), 404
...
def update_patient(id):
    ...
    if has_access:
        try:
            ...
            updatePatientGeneralStatus = PatientGeneralStatus(patient_id, general_condition, energy, fever, chest_pain, dizziness, high_temperature, sweating, 
                 palpitations, resting_tachycardia, falls, instability, change_of_location, exercise_difficulty, dyspnea_activities)
            
            updatePatientHabitsAndBackgroud = PatientHabitsAndBackgroud(patient_id, fruits_vegetables, water, physical_activity, sleep_hours, nighttime_waking, medical_history, medications, 
                                                                     cardiac_history, respiratory_history, obesity, family_diabetes, diabetes, chronic_disease, disease_details)
            
            affected_rows = PatientModel.add_patient(updatePatient,updatePatientGeneralStatus,updatePatientHabitsAndBackgroud)

            if affected_rows == 1:
                return jsonify(id)
            else:
                return jsonify({"message": "Patient updated"}), 404
